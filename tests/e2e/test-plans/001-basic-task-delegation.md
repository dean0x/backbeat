...
## Test Description
Verify that Autobeat can successfully delegate a simple task to a background Claude Code instance and retrieve the results.
...
**Action:** Check if Autobeat CLI is available
```bash
...
```bash
mkdir -p .autobeat && rm -f .autobeat/autobeat.db
```
**Expected:** Database directory created
**Verify:**
- .autobeat directory exists
...
```bash
ls -la .autobeat/autobeat.db 2>/dev/null || echo "Database not created"
```
