...
```bash
rm -rf .autobeat/autobeat.db .autobeat/autobeat.db-wal .autobeat/autobeat.db-shm
```
**Expected:** Clean slate for testing
**Verify:**
- No database files exist
- .autobeat directory is clean
...
```bash
ls -la .autobeat/autobeat.db* && sqlite3 .autobeat/autobeat.db "PRAGMA journal_mode;" 2>/dev/null || echo "Database check failed"
```
**Expected:** Database with WAL mode
**Verify:**
- autobeat.db file exists
- WAL mode is enabled (output should show "wal")

### Step 5: Check Task in Database
**Action:** Query database directly for task
```bash
sqlite3 .autobeat/autobeat.db "SELECT COUNT(*) FROM tasks;" 2>/dev/null || echo "0"
```
...
**Action:** Kill all autobeat processes abruptly
```bash
pkill -9 -f "autobeat" || true
sleep 2
```
**Expected:** Processes terminated
**Verify:**
- No autobeat processes running
- Database files still exist

### Step 8: Check Database Integrity
**Action:** Verify database is not corrupted
```bash
sqlite3 .autobeat/autobeat.db "PRAGMA integrity_check;" 2>/dev/null || echo "integrity check failed"
```
...
```bash
sqlite3 .autobeat/autobeat.db "SELECT id, status, priority FROM tasks ORDER BY createdAt;" 2>/dev/null || echo "Query failed"
```
...
```bash
sqlite3 .autobeat/autobeat.db "SELECT COUNT(*) FROM tasks;" 2>/dev/null
```
...
## Rollback Plan
If test fails:
1. Remove corrupted database: `rm -rf .autobeat/autobeat.db*`
